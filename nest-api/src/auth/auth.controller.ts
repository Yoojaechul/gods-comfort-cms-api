...
export class AuthController {
  ...
  async login(@Body() loginDto: LoginDto, @Req() req: Request) {
    ...
    const maskedLoginDto = {
      ...
      password: loginDto.password
        ? `*** (length=${loginDto.password.length})`
        : undefined,
    };
    ...
    try {
      ...
      this.logger.log(
        `[LOGIN] 로그인 성공: ${result.user?.id} (${result.user?.role})`,
      );
      return result;
    } catch (error) {
      // 상세 에러 로깅 (스택 트레이스 포함)
      ...
      this.logger.error(
        `[LOGIN] 에러 타입: ${error?.constructor?.name || "Unknown"}`,
      );
      this.logger.error(
        `[LOGIN] 에러 메시지: ${error instanceof Error ? error.message : String(error)}`,
      );
      ...
      if (error instanceof Error) {
        if (error.message.includes("Invalid username/email or password")) {
          this.logger.warn(
            `[LOGIN] 인증 실패 - 사용자 없음 또는 비밀번호 불일치: ${identifier}`,
          );
  ...
  @ApiResponse({
    status: 400,
    description: "현재 비밀번호가 올바르지 않음 또는 잘못된 요청",
  })
  ...
  async changePassword(@Body() changePasswordEmailDto: ChangePasswordEmailDto) {
    ...
    try {
      ...
      if (!result.ok) {
        ...
        // 그 외의 경우 400
        this.logger.warn(
          `[CHANGE_PASSWORD] 실패: ${email} - ${result.message}`,
        );
      ...
      this.logger.error(
        `[CHANGE_PASSWORD] 에러 타입: ${error?.constructor?.name || "Unknown"}`,
      );
      this.logger.error(
        `[CHANGE_PASSWORD] 에러 메시지: ${error instanceof Error ? error.message : String(error)}`,
      );
  ...
  async getSeedStatus() {
    ...
    if (isProduction && !isDebugEnabled) {
      this.logger.warn(
        "[SEED_STATUS] 프로덕션 환경에서 CMS_DEBUG=true 없이 접근 시도",
      );
