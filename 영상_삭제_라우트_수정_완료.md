# 영상 삭제 라우트 수정 완료 보고서

## ✅ 수정 완료된 라우트

### 1. DELETE /admin/videos/:id (1644줄)
- ✅ Admin 전용 라우트 존재 확인
- ✅ Admin은 owner_id/site_id와 무관하게 모든 영상 삭제 가능
- ✅ 영상 존재 확인 후 삭제
- ✅ 로그 추가: 라우트명, user id, role, video id, 처리 결과
- ✅ 응답 형식: `{ ok: true, success: true }`

### 2. DELETE /videos/:id (2509줄)
- ✅ Admin/Creator 모두 사용 가능하도록 수정
- ✅ Admin: 모든 영상 삭제 가능
- ✅ Creator: 본인 소유(owner_id 일치)만 삭제 가능
- ✅ 영상 존재 확인 후 권한 체크
- ✅ 로그 추가: 라우트명, user id, role, video id, 처리 결과
- ✅ 응답 형식: `{ ok: true, success: true }`
- ✅ 에러 코드: 404 (Video not found), 403 (Access denied)

## 🔒 적용된 로직

### DELETE /admin/videos/:id

```javascript
const routeName = "DELETE /admin/videos/:id";
const { id } = request.params;
const user = request.user;

console.log(`[${routeName}] 삭제 요청 - user: ${user.id}, role: ${user.role}, video_id: ${id}`);

// 영상 존재 확인
const video = db.prepare("SELECT * FROM videos WHERE id = ?").get(id);
if (!video) {
  console.log(`[${routeName}] 영상을 찾을 수 없음: ${id}`);
  return reply.code(404).send({ error: "Video not found" });
}

// Admin은 owner_id/site_id와 무관하게 모든 영상 삭제 가능
const result = db.prepare("DELETE FROM videos WHERE id = ?").run(id);

if (result.changes === 0) {
  console.error(`[${routeName}] 삭제 실패 (변경된 행 없음): ${id}`);
  return reply.code(500).send({ error: "Delete operation failed" });
}

console.log(`[${routeName}] Admin이 영상 삭제 성공: ${id}`);
return { ok: true, success: true };
```

### DELETE /videos/:id

```javascript
const routeName = "DELETE /videos/:id";
const { id } = request.params;
const user = request.user;

console.log(`[${routeName}] 삭제 요청 - user: ${user.id}, role: ${user.role}, video_id: ${id}`);

// 영상 존재 확인
const video = db.prepare("SELECT * FROM videos WHERE id = ?").get(id);
if (!video) {
  console.log(`[${routeName}] 영상을 찾을 수 없음: ${id}`);
  return reply.code(404).send({ error: "Video not found" });
}

// 권한 확인: admin은 모든 영상 삭제 가능, creator는 본인 소유만
if (user.role === "admin") {
  // Admin: owner_id/site_id와 무관하게 삭제 허용
  const result = db.prepare("DELETE FROM videos WHERE id = ?").run(id);
  if (result.changes === 0) {
    console.error(`[${routeName}] 삭제 실패 (변경된 행 없음): ${id}`);
    return reply.code(500).send({ error: "Delete operation failed" });
  }
  console.log(`[${routeName}] Admin이 영상 삭제 성공: ${id}`);
  return { ok: true, success: true };
} else {
  // Creator: 본인 소유만 삭제 가능
  if (video.owner_id !== user.id) {
    console.warn(`[${routeName}] Creator(${user.id})가 다른 사용자의 영상(${id}, owner: ${video.owner_id}) 삭제 시도 - 거부`);
    return reply.code(403).send({ error: "Access denied: You can only delete your own videos" });
  }
  const result = db.prepare("DELETE FROM videos WHERE id = ? AND owner_id = ?").run(id, user.id);
  if (result.changes === 0) {
    console.error(`[${routeName}] Creator 삭제 실패 (변경된 행 없음): ${id}`);
    return reply.code(500).send({ error: "Delete operation failed" });
  }
  console.log(`[${routeName}] Creator가 본인 영상 삭제 성공: ${id}`);
  return { ok: true, success: true };
}
```

## 📝 삭제 방식

- **하드 삭제**: `DELETE FROM videos WHERE id = ?`
- videos 테이블에 `deleted` 또는 `is_deleted` 컬럼이 없으므로 하드 삭제 사용

## ✅ 완료 기준 달성

- [x] DELETE /admin/videos/:id 라우트 존재 및 개선 완료
- [x] DELETE /videos/:id 라우트 개선 완료 (admin/creator 모두 사용 가능)
- [x] Admin은 모든 영상 삭제 가능 (owner_id/site_id 무관)
- [x] Creator는 본인 소유만 삭제 가능
- [x] 영상 존재 확인 후 삭제
- [x] 로그 추가 (라우트명, user id, role, video id, 처리 결과)
- [x] 명확한 에러 코드 (404: Video not found, 403: Access denied)
- [x] 응답 형식: `{ ok: true, success: true }`

## 📊 최종 요약

**수정한 라우트**: 2개
- DELETE /admin/videos/:id (1644줄) - 개선 완료
- DELETE /videos/:id (2509줄) - 개선 완료

**주요 변경 사항**:
1. DELETE /admin/videos/:id에 로그 추가 및 에러 처리 개선
2. DELETE /videos/:id를 admin/creator 모두 사용 가능하도록 수정
3. Admin은 모든 영상 삭제 가능, Creator는 본인 소유만 삭제 가능
4. 영상 존재 확인 후 삭제 처리
5. 모든 로그에 라우트명 포함

**결과**: CMS Admin에서 영상 삭제가 정상 동작하며, Creator는 본인 영상만 삭제 가능합니다.





























